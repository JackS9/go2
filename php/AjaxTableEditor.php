...
class AjaxTableEditor
{
	...
	public $instanceName;
	public $editInPlace;
	public $editInPlaceFun;
	public $addInPlace;
	public $addInPlaceFun;
	public $valErrorFields = array();
	...
	public function setDefaults()
	{
		...
		$this->defaultUploadMaxSize = isset($this->defaultUploadMaxSize) ? $this->defaultUploadMaxSize : 47185920;
		...
		$this->editInPlace = isset($this->editInPlace) ? $this->editInPlace : false;
		$this->addInPlace = isset($this->addInPlace) ? $this->addInPlace : false;
		$this->persistentAddForm = isset($this->persistentAddForm) ? $this->persistentAddForm : $this->addInPlace;
		...
		$this->fileUploading = isset($this->fileUploading) ? $this->fileUploading : false;
	...
	public function main()
	{
		...
		{
			echo '<script type="text/javascript">parent.'.$this->instanceName.'.displayInfo('.$this->jsonEncode($this->retArr).');</script>';
		}
		else
		{
	...
	public function handleFlow()
	{
		switch ($this->action)
		{
			...
			case 'edit_in_place':
				$this->editInPlace();
				break;
			case 'display_row_in_place':
				$this->displayRowInPlace($this->info['id'],$this->info['mateRowNum'],$this->info['newRow']);
				break;
			case 'edit_rows_in_place':
				$this->editRowsInPlace();
				break;
			case 'update_rows_in_place':
				$this->updateRowsInPlace();
				break;
			case 'cancel_rows_in_place':
				$this->cancelRowsInPlace();
				break;
			default :
				$this->doDefault();
		}
	...
	public function showHideColumns()
	{
		if(stristr($this->permissions,'H'))
		{
			...
			$this->setInnerHtml('updateInPlaceLayer','');
			//$this->addJavascript("resetScrollTop();");
	...
	public function formatIcons($id,$row,$mateRowNum)
	{
		...
		if(stristr($this->permissions,'E') && stristr($this->removeIcons,'E') === false)
		{
			if($this->editInPlace)
			{
				$html .= '<li class="edit"><a href="javascript: void(0);" onclick="'.$this->instanceName.'.editInPlace(\''.$id.'\',\''.$mateRowNum.'\');" title="'.$this->langVars->ttlEdit.'"></a></li>';
			}
			else
			{
	...
	public function updateRow()
	{
		...
		$this->oldPrimaryKeyValue = $oldPrimaryKeyValue;
		
		$hasFileUpload = isset($this->info['has_file_upload']) ? $this->info['has_file_upload'] : false;
		$this->fileUploading = $hasFileUpload;
		
		unset($this->info['old_primary_key_value'],$this->info['has_file_upload']);
		
		$this->info = $this->stripMateInstanceName($this->info);
		
		if($hasFileUpload && isset($_FILES))
		{
			$_FILES = $this->stripMateInstanceName($_FILES);
		}
		
		$newPrimaryKeyValue = $this->updateRowInDb($this->info,$oldPrimaryKeyValue);
		
		if(strlen($newPrimaryKeyValue) > 0 && !$this->valError && $hasFileUpload)
		{
			$this->callFileUploadFuns($newPrimaryKeyValue,'',false);
		}
	...
	public function updateRowInDb($info,$oldPrimaryKeyValue,$mateRowNum='',$inPlace=false)
	{
		...
		foreach($info as $col => $val)
		{
			if(isset($this->tableColumns[$col]))
			{
				...
				$val = $this->getSqlValue($col,$val);
				if($val === false && !isset($this->tableColumns[$col]['mysql_edit_fun']))
				{
					continue;
				}
				if(isset($this->tableColumns[$col]['file_upload']))
				{
          if(!isset($this->tableColumns[$col]['file_upload']['upload_fun']))
          {
					    $insertContent = true;
					    foreach($fileColumns as $fileCol)
					    {
						    if(isset($this->tableColumns[$col]['file_upload'][$fileCol]) && isset($_FILES[$col][$fileCol]))
					      {
							    $val = $_FILES[$col][$fileCol];
							    $preparedSets[] = $this->getPreparedSet($this->tableColumns[$col]['file_upload'][$fileCol]);
							    if($val !== null)
							    {
								    $queryParams[$this->tableColumns[$col]['file_upload'][$fileCol]] = $val;
							    }
							    $afterEditArray[$this->tableColumns[$col]['file_upload'][$fileCol]] = $val;
						    }
              }
          }
          else
              $insertContent = false;
					...
					if(isset($_FILES[$col]['tmp_name']) && strlen($_FILES[$col]['tmp_name']) > 0)
					{
						//$preparedSets[] = $this->getAddEditPreparedSet($col,'','edit');
						$preparedSets[] = $this->getPreparedSet($col);
						if($insertContent)
							$queryParams[$col] = file_get_contents($_FILES[$col]['tmp_name']);
						else
							$queryParams[$col] = $_FILES[$col]['tmp_name'];
						$afterEditArray[$col] = $_FILES[$col]['tmp_name'];
					}
				}
				else
				{
	...
	public function insertRow()
	{
		...
		$hasFileUpload = isset($this->info['has_file_upload']) ? $this->info['has_file_upload'] : false;
		$this->fileUploading = $hasFileUpload;
		unset($this->info['has_file_upload']);
		$this->info = $this->stripMateInstanceName($this->info);
		if($hasFileUpload && isset($_FILES))
		{
			$_FILES = $this->stripMateInstanceName($_FILES);
		...
		if(strlen($insertId) > 0 && !$this->valError && $hasFileUpload)
		{
			$this->callFileUploadFuns($insertId,'',true);
		}
	...
	public function insertRowInDb($info,$insertId,$nextRowNum='',$inPlace=false)
	{
		...
		foreach($info as $col => $val)
		{
			if(isset($this->tableColumns[$col]))
			{
				...
				if(isset($this->tableColumns[$col]['file_upload']) && isset($_FILES[$col]))
				{
          if(!isset($this->tableColumns[$col]['file_upload']['upload_fun']))
          {
					  $insertContent = true;
					  foreach($fileColumns as $fileCol)
					  {
						  ...
						  {
						  	$val = $_FILES[$col][$fileCol];
							  //$preparedSets[] = $this->getAddEditPreparedSet($this->tableColumns[$col]['file_upload'][$fileCol],$val,'add');
							  $preparedSets[] = $this->getPreparedSet($this->tableColumns[$col]['file_upload'][$fileCol]);
	...
	public function validateInputValue($col,$val,$row,$addOrEdit,$inPlace=false)
	{
		...
		if(isset($this->tableColumns[$col]['file_upload']['max_size']) && isset($_FILES[$col]))
		{		
			$maxSize = isset($this->tableColumns[$col]['file_upload']['max_size']) ? $this->tableColumns[$col]['file_upload']['max_size'] : $this->defaultUploadMaxSize;
			if($_FILES[$col]['size'] > $maxSize)
			{
				$valError = true;
				$this->addTooltipMsg(sprintf($this->langVars->errFileSize,$this->tableColumns[$col]['display_text']));
			}
		}
		...
		if(stristr($this->permissions,'A'))
		{
			...
			foreach($this->tableColumns as $col => $info)
			{
				if(stristr($info['perms'],'A') && !isset($info['join']['real_column']))
				{
					...
					$hasFileUpload = $hasFileUpload == true ? $hasFileUpload : $inputHtmlInfo['has_file_upload'];
			...
			if($hasFileUpload)
			{
				$formInfo = 'method="post" action="'.$_SERVER['REQUEST_URI'].'" enctype="multipart/form-data" target="'.$this->instanceName.'file_upload_frame" onSubmit="return false;"';
				$iFrameHtml .= $this->getIframeHtml();
			}
	...
	public function getInputDefaultValue($info,$row,$col,$addOrEdit,$mateRowNum='')
	{
		...
		if(isset($val) && strlen($val) > 0)
		{
			...
			{
				$defVal = call_user_func($info[$addOrEdit.'_fun'],$col,$defVal,$row,$this->instanceName,$mateRowNum);
			}
			...
			else if(isset($info['file_upload']))
			{
				if(isset($info['file_upload']['name']) && isset($row[$info['file_upload']['name']]))
				{
					$defVal = $row[$info['file_upload']['name']];
				}
				else if(isset($info['display_mask']) && isset($row[$info['display_mask']]))
				{
					$defVal = $row[$info['display_mask']];
				}
				else if(isset($info['file_upload']['upload_fun']) && isset($row[$col]))
				{
					$defVal = $row[$col];
				}
			}
	...
	public function getInputHtmlInfo($info,$row,$col,$jsFun,$addOrEdit,$editMult=false,$mateRowNum='')
	{
		...
		{
			$deleteIcon = '';
			if($addOrEdit == 'edit' && isset($info['file_upload']['delete_fun']) && is_callable($info['file_upload']['delete_fun']))
			{
				$deleteIcon = '<div style="display: inline-block;"><ul class="actions"><li class="delete"><a href="javascript: void(0);" onclick="'.$this->instanceName.'.confirmDeleteUpload(\''.$this->escapeJsData($defVal).'\',\''.$this->escapeJsData($row[$this->primaryKeyCol]).'\',\''.$this->escapeJsData($col).'\',\''.$this->escapeJsData($inputId).'\');" title="Delete '.htmlspecialchars($defVal).'"></a></li></ul></div>';
			}
			$defVal = strlen($defVal) > 0 ? trim(htmlspecialchars($defVal).' '.$deleteIcon).'<br />' : '';
			$inputHtml .= '<span id="'.$this->instanceName.$inputId.'delete_upload">'.$defVal.'</span><input type="file" name="'.$inputId.'" id="'.$inputId.'" '.$inputInfo.' />';
			$hasFileUpload = true;
		}
		...
		return array('input_html' => $inputHtml, 'has_file_upload' => $hasFileUpload);
	}
	...
	public function displayBottomInfo()
	{
		...
		if(stristr($this->permissions,'A'))
		{
			if($this->addInPlace)
			{
				if($this->persistentAddForm)
				{
					$this->addInPlace(1);
					//$this->addJavascript($this->instanceName.'.toAjaxTableEditor(\'add_in_place\',\'\');',true);
				}
				else
				{
					$html .= '<button onclick="'.$this->instanceName.'.toAjaxTableEditor(\'add_in_place\',\'\');">'.$this->langVars->btnAdd.'</button>';
				}
				//$html .= '<button onclick="'.$this->instanceName.'.editRowsInPlace();">'.$this->langVars->btnEdit.'</button>';
			}
			else
			{
		...
		$btnHtml = $this->getInPlaceBtnHtml();
		$this->setInnerHtml('updateInPlaceLayer',$btnHtml);
		$this->addUpdateInPlaceBtnsJs();
	}	
			...
			foreach($this->tableColumns as $col => $info)
			{
				if(stristr($info['perms'],'T') && !$this->columnIsHidden($info))
				{
					...
					if(stristr($this->permissions,'F') && stristr($info['perms'],'F'))
					{
						...
						if(isset($info['filter_type']))
						{
							$colHeaderInfo .= ' filterType="'.$info['filter_type'].'"';
						}
					}
	...
	public function drawRowInPlace($id,$mateRowNum,$newRow,$drawAfterRowNum='')
	{
		...
		if(count($row) > 0)
		{
			...
			$this->rowsDrawnInPlace[] = array('id' => $id, 'mate_row_num' => $mateRowNum, 'new_row' => $newRow, 'draw_after_row_num' => $drawAfterRowNum);
		}
	}
	...
	public function editInPlace()
	{
		if(stristr($this->permissions,'E'))
		{
			...
			$rowHtml = '';
			$id = $this->info['id'];
			$mateRowNum = $this->info['mateRowNum'];
			$jsFun = $this->instanceName.'.updateRowsInPlace(\''.$id.'\',\''.$mateRowNum.'\');';
			$formInfo = '';
			$iFrameHtml = '';
			$hasFileUpload = false;
			...
			$this->formatJoinClause();
			$this->formatSelectClause();
			$displayRow = $this->getDisplayRowInfo($id);
			$editRow = $this->getTableRow($id);
			$row = array();
			foreach($this->tableColumns as $col => $info)
			{
				if(isset($info['join']) && !stristr($info['perms'],'E') && stristr($info['perms'],'T') && isset($displayRow[$col])) 
				{
					$row[$col] = $displayRow[$col];
				}
				else if(isset($info['display_mask']) && !stristr($info['perms'],'E') && stristr($info['perms'],'T') && isset($displayRow[$col]))
				{
					$row[$col] = $displayRow[$col];
				}
				else if(isset($editRow[$col]))
				{
					$row[$col] = $editRow[$col];
				}
			}			
			...
			if(count($row) > 0)
			{
				if(isset($this->beforeEditFun) && is_callable($this->beforeEditFun))
				{
					$returnVal = call_user_func($this->beforeEditFun,$this->primaryKeyCol,$id,$row,$this->instanceName,$mateRowNum);
					if($returnVal === false)
					{
						return false;
					}
				...
				foreach($this->tableColumns as $col => $info)
				{
					// If the column is displayed but not editable
					//if(!stristr($info['perms'],'E') && stristr($info['perms'],'T'))
					//{
					//	$rowHtml .= $this->getTableCellHtml($row,$col,$info);
					//}
					if(stristr($info['perms'],'E') && !isset($info['join']['real_column']))
					{
						$inputHtmlInfo = $this->getInputHtmlInfo($info,$row,$col,$jsFun,'edit',false,$mateRowNum);
						$inputHtml = $inputHtmlInfo['input_html'];
						$hasFileUpload = $hasFileUpload == true ? $hasFileUpload : $inputHtmlInfo['has_file_upload'];
						if((isset($info['hidden_edit']) && $info['hidden_edit']) && ($this->columnIsHidden($info) || !stristr($info['perms'],'T')))
						{
							$rowHtml .= $inputHtml;
						}
						else if(stristr($info['perms'],'T'))
						{
							$rowHtml .= '<td nowrap="nowrap">'.$inputHtml.'</td>';
						}
					}
					else if(stristr($info['perms'],'T') && !$this->columnIsHidden($info))
					{
						$rowHtml .= $this->getTableCellHtml($row,$col,$info,$mateRowNum);
					}
				}
				if($hasFileUpload)
				{
					$formInfo = 'method="post" action="'.$_SERVER['REQUEST_URI'].'" enctype="multipart/form-data" target="'.$this->instanceName.'file_upload_frame'.$mateRowNum.'" onSubmit="return false;"';
					$iFrameHtml .= $this->getIframeHtml($mateRowNum);
				}
			}
			$formId = $this->instanceName.'add_edit_form'.$mateRowNum;
			$formHtml = '<form style="display: none;" id="'.$formId.'" name="'.$formId.'" '.$formInfo.'>';
			$formHtml .= '</form>';
			$cbInfo = $this->getCbInfo($row,$mateRowNum,true);
			$html .= '<tr id="'.$this->instanceName.'row_'.$mateRowNum.'" class="editInPlaceRow" row_num="'.$mateRowNum.'" row_id="'.htmlspecialchars($id).'">';
			$html .= $cbInfo['cb_html'];
			if(stristr($this->permissions,'I') !== false && $this->iconColPosition == 'first')
			{
				$html .= '<td>'.$this->getEditInPlaceIcons($id,$mateRowNum).'</td>';
			}
			$html .= $rowHtml;
			if(stristr($this->permissions,'I') !== false && $this->iconColPosition == 'last')
			{
				$html .= '<td>'.$this->getEditInPlaceIcons($id,$mateRowNum).'</td>';
			}
			$html .= '</tr>';
			$this->replaceHtml('row_'.$mateRowNum,$html);
			$this->addJavascript($this->instanceName.'.addHiddenForm(\''.$formId.'\',\''.$this->escapeJsData($formHtml.$iFrameHtml).'\');');
			
			if(is_callable($this->editInPlaceFun))
			{
				call_user_func($this->editInPlaceFun,$this->instanceName,$mateRowNum);
			}
			return true;
		}
	}
	
	public function editRowsInPlace()
	{
		$info = $this->info;
		unset($this->info);
		foreach($info as $mateRowNum => $rowId)
		{
			$this->info['id'] = $rowId;
			$this->info['mateRowNum'] = $mateRowNum;
			$this->editInPlace();
		}
		$this->addUpdateInPlaceBtnsJs();	
	}
	
	public function updateRowsInPlace()
	{
		$info = $this->info;
		unset($this->info);
		
		// Set hidden columns, column order etc for re-drawing the row(s) in place
		$this->setHiddenColumns();
		$this->setColumnOrder();
		$this->formatJoinClause();
		$this->formatSelectClause();
		...
		$this->finishDrawRowsInPlace();
		$this->addUpdateInPlaceBtnsJs();
	}
	
	public function cancelRowsInPlace()
	{
		$info = $this->info;
		unset($this->info);
		$this->setHiddenColumns();
		...
		$this->formatJoinClause();
		$this->formatSelectClause();
		foreach($info['uipRows'] as $mateRowNum => $rowId)
		{
			$this->drawRowInPlace($rowId,$mateRowNum,false);
		}
		$this->finishDrawRowsInPlace();
		$this->addUpdateInPlaceBtnsJs();
	}
	
	public function getIframeHtml($mateRowNum='')
	{
		$iFrameHtml = '<div style="display: none">';
		$iFrameHtml .= '<iframe src="" name="'.$this->instanceName.'file_upload_frame'.$mateRowNum.'" id="'.$this->instanceName.'file_upload_frame'.$mateRowNum.'"></iframe>';
		$iFrameHtml .= '</div>';
		return $iFrameHtml;
	}
	
	public function handleDeleteUpload()
	{
		if(isset($this->tableColumns[$this->info['col']]['file_upload']['delete_fun']) && is_callable($this->tableColumns[$this->info['col']]['file_upload']['delete_fun']))
		{
			$info = $this->info;
			if(call_user_func($this->tableColumns[$this->info['col']]['file_upload']['delete_fun'],$info,$this->instanceName))
			{
				$this->setInnerHtml($this->info['inputId'].'delete_upload','');
			}
		}
	}
	
	public function getAddInPlaceIcons()
	{
		$html = '';
		$width = 26;
		$html .= '<li class="add"><a href="javascript: void(0);" onclick="'.$this->instanceName.'.insertInPlace();" title="'.$this->langVars->btnAdd.'"></a></li>';
		if(!$this->persistentAddForm)
		{
			$html .= '<li class="undo"><a href="javascript: void(0);" onclick="'.$this->instanceName.'.cancelAddInPlace();" title="'.$this->langVars->btnCancel.'"></a></li>';
			$width = 52;
		}
		return '<ul class="actions" style="width: '.$width.'px;">'.$html.'</ul>';
	}
	
	public function getEditInPlaceIcons($id,$mateRowNum)
	{
		$html = '';
		$html .= '<li class="save"><a href="javascript: void(0);" onclick="'.$this->instanceName.'.updateRowsInPlace(\''.$id.'\',\''.$mateRowNum.'\');" title="'.$this->langVars->btnUpdate.'"></a></li>';
		$html .= '<li class="undo"><a href="javascript: void(0);" onclick="'.$this->instanceName.'.cancelRowsInPlace(\''.$id.'\',\''.$mateRowNum.'\');" title="'.$this->langVars->btnCancel.'"></a></li>';
		return '<ul class="actions" style="width: 52px;">'.$html.'</ul>';
	}
	...
	public function updateInPlace()
	{
		...
		$hasFileUpload = isset($this->info['inputInfo']['has_file_upload']) ? $this->info['inputInfo']['has_file_upload'] : false;
		$this->fileUploading = $hasFileUpload;
		
		unset($this->info['old_primary_key_value'],$this->info['inputInfo']['has_file_upload']);
		
		$this->setHiddenColumns();		
		
		$inputInfo = $this->stripMateInstanceName($this->info['inputInfo'],$mateRowNum);

		if($hasFileUpload && isset($_FILES))
		{
			$_FILES = $this->stripMateInstanceName($_FILES,$mateRowNum);
		}
		
		$newPrimaryKeyValue = $this->updateRowInDb($inputInfo,$oldPrimaryKeyValue,$mateRowNum,true);
		if($newPrimaryKeyValue === false || strlen(trim($newPrimaryKeyValue)) == 0) {
			$this->valError = true;
		}
		
		if(strlen($newPrimaryKeyValue) > 0 && !$this->valError && $hasFileUpload)
		{
			$this->callFileUploadFuns($newPrimaryKeyValue,$mateRowNum,false);
		}
		
		if($this->valError)
		{
			$this->addJavascript($this->instanceName.'.displayFormInputs(\''.$mateRowNum.'\');');
			if($this->showDefaultValidationMsg)
			{
				$this->addTooltipMsg($this->langVars->errValInPlace.': '.implode(', ',$this->valErrorFields));
			}
			$columns = array_keys($this->valErrorFields);
			if(isset($columns[0])) {
				$this->addJavascript('$(\'#'.$this->instanceName.$columns[0].'_'.$mateRowNum.'\').focus();');
			}
		}
		else
		{
			$this->drawRowInPlace($newPrimaryKeyValue,$mateRowNum,false);
			$this->addUpdateInPlaceBtnsJs();
		}
	}
	
	public function finishDrawRowsInPlace()
	{
		//$this->clearInPlaceValErrs();
		if(isset($this->rowsDrawnInPlace) && count($this->rowsDrawnInPlace) > 0 && isset($this->afterRowsDrawnInPlaceFun) && is_callable($this->afterRowsDrawnInPlaceFun))
		{
			call_user_func($this->afterRowsDrawnInPlaceFun,$this->rowsDrawnInPlace,$this->instanceName);
		}
	}
	
	public function displayRowInPlace($id,$mateRowNum,$newRow=false,$drawAfterRowNum='')
	{
		$this->setHiddenColumns();
		$this->setColumnOrder();
		$this->formatJoinClause();
		$this->formatSelectClause();
		$this->drawRowInPlace($id,$mateRowNum,$newRow,$drawAfterRowNum);
		$this->finishDrawRowsInPlace();
	}
	
	public function insertInPlace()
	{
		$this->valError = false;
		$hasFileUpload = isset($this->info['has_file_upload']) ? $this->info['has_file_upload'] : false;
		$this->fileUploading = $hasFileUpload;
		$nextRowNum = $this->info['last_row_num'] + 1;
		unset($this->info['last_row_num'],$this->info['has_file_upload']);
		...
		$insertId = isset($this->info[$this->primaryKeyCol]) ? $this->info[$this->primaryKeyCol] : '';
		$insertId = $this->insertRowInDb($this->info,$insertId,$nextRowNum,$inPlace=true);

		if(strlen($insertId) > 0 && !$this->valError && $hasFileUpload)
		{
			$this->callFileUploadFuns($insertId,$nextRowNum,true);
		}
		...
		if($this->valError)
		{
			...
			$this->addJavascript($this->instanceName.'.displayFormInputs();');
			$columns = array_keys($this->valErrorFields);
			$this->addJavascript('$(\'#'.$this->instanceName.$columns[0].'\').focus();');
		}
		else
		{
			$this->setHtmlValue('last_row_num',$nextRowNum);
			$this->addJavascript($this->instanceName.'.cancelAddInPlace();');
			if($this->persistentAddForm)
			{
				$this->addInPlace(1);
			}
			$this->displayRowInPlace($insertId,$nextRowNum,true);
		}
		return $insertId;
	}
	
	public function addInPlace($delay=0)
	{
		if(stristr($this->permissions,'A'))
		{
			$html = '';
			$rowHtml = '';
			$jsFun = $this->instanceName.'.insertInPlace();';
			$formInfo = '';
			$iFrameHtml = '';
			$hasFileUpload = false;
			...
			foreach($this->tableColumns as $col => $info)
			{
				if(stristr($info['perms'],'A') && !isset($info['join']['real_column']))
				{
					...
					$inputHtml = $inputHtmlInfo['input_html'];
					$hasFileUpload = $hasFileUpload == true ? $hasFileUpload : $inputHtmlInfo['has_file_upload'];
					if((isset($info['hidden_add']) && $info['hidden_add']) && ($this->columnIsHidden($info) || !stristr($info['perms'],'T')))
					{
						$rowHtml .= $inputHtml;
					}
					else if(stristr($info['perms'],'T'))
			...
			$formId = $this->instanceName.'add_edit_form';
			$formHtml = '<form style="display: none;" id="'.$formId.'" name="'.$formId.'" '.$formInfo.'>';
			$formHtml .= '</form>';
			//$cbInfo = $this->getCbInfo(,$mateRowNum,true);
			$html .= '<tr id="'.$this->instanceName.'add_in_place_row" class="addInPlaceRow">';
			if(stristr($this->permissions,'M'))
			{
				$html .= '<td>&nbsp;</td>';
			}
			if(stristr($this->permissions,'I') !== false)
			{
				$iconHtml = '<td>'.$this->getAddInPlaceIcons().'</td>';
				$html = $this->iconColPosition == 'first' ? $html.$iconHtml.$rowHtml : $html.$rowHtml.$iconHtml;
				//$html .= $rowHtml;
			}
			else if(stristr($this->permissions,'M') !== false)
			{
				$html .= $rowHtml;
				//$btnHtml = $this->getInPlaceBtnHtml();
				//$this->setInnerHtml('updateInPlaceLayer',$btnHtml);
			...
			$html .= '</tr>';
			$this->addJavascript($this->instanceName.'.insertAddInPlaceForm(\''.$this->escapeJsData($html).'\','.$this->returnJsBool(stristr($this->permissions,'F') !== false).',\''.$this->filterPosition.'\');',$delay);
			$this->addJavascript($this->instanceName.'.addHiddenForm(\''.$formId.'\',\''.$this->escapeJsData($formHtml.$iFrameHtml).'\');',$delay);
			$this->addUpdateInPlaceBtnsJs();
			if(is_callable($this->addInPlaceFun))
			{
				call_user_func($this->addInPlaceFun,$this->instanceName);
			}
		}
	}
	
	public function addUpdateInPlaceBtnsJs()
	{
		$this->addJavascript($this->instanceName.'.updateInPlaceBtns('.$this->returnJsBool(stristr($this->permissions,'A') !== false).','.$this->returnJsBool(stristr($this->permissions,'E') !== false).','.$this->returnJsBool(stristr($this->permissions,'M') !== false).','.$this->returnJsBool(stristr($this->permissions,'I') !== false).','.$this->returnJsBool($this->persistentAddForm).');');
	}
	
	public function getInPlaceBtnHtml()
	{
		$html = '<div class="addInPlaceBtnsDiv">';
		$html .= '<button onclick="'.$this->instanceName.'.insertInPlace();">'.$this->langVars->btnAdd.'</button>';
		if(!$this->persistentAddForm)
		{
			$html .= '<button onclick="'.$this->instanceName.'.cancelAddInPlace();">'.$this->langVars->btnCancel.'</button>';
		}
		$html .= '</div>';
		$html .= '<div class="editInPlaceBtnsDiv"><button onclick="'.$this->instanceName.'.updateRowsInPlace();">'.$this->langVars->btnUpdate.'</button>&nbsp;<button onclick="'.$this->instanceName.'.cancelRowsInPlace();">'.$this->langVars->btnCancel.'</button></div>';
		return $html;
	}
	
	public function callFileUploadFuns($id,$mateRowNum,$newRow)
	{
		foreach($this->tableColumns as $col => $colInfo)
		{
			if(isset($colInfo['file_upload']['upload_fun']) && isset($_FILES[$col]) && is_callable($colInfo['file_upload']['upload_fun']))
			{
				$valErrors = call_user_func($colInfo['file_upload']['upload_fun'],$col,$id,$_FILES[$col],$valErrors,$this->instanceName);
			}
		}
	}
