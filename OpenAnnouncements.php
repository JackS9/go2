...
class OpenAnnouncements extends Common
{
    protected $Editor;
    ...
    protected function initiateEditor()
    {
        ...
        $tableColumns['ann_datetimestamp'] = array(
            ...
            'order_mask' => 'tbl_MARS_Announce.ann_datetimestamp',
            'range_mask' => 'tbl_MARS_Announce.ann_datetimestamp'
        );
        if ($user->isLoggedIn && ($user->isAdmin && isset($_GET['asAdmin'])) || ($user->isManager && isset($_GET['asManager'])))
        {
            $tableColumns['submitted_proposals'] = array(
                ...
                'perms' => 'VXQSHO',
                'display_mask' => "GROUP_CONCAT(proposal.proposal_Name SEPARATOR \"<hr>\")",
        ...
        $this->Editor->setConfig('tableTitle','Funding Opportunity Announcements (Open RFPs)');
        ...
        if ($user->isLoggedIn)
        {
            ...
            $userActions['submit_proposal'] = array(&$this,'submitProposal');
        ...
        $this->Editor->setConfig('sqlFilters',"ann_Date_Close >= NOW()");
    ...
    public function displayHtml()
    {
        ...
        $defaultSessionData['orderByColumn'] = 'ann_ID';
    ...
    public function formatProposals($col,$val,$row)
    {
        ...
        if (strlen($val) > 0)
        {
            $html .= '<textarea readonly="readonly" rows="5" cols="60">'.$val.'</textarea>';
            if ($user->isLoggedIn && $this->Editor->action == 'view_row')
            {
                $html .= '<hr>';
                $html .= '<img style="border: none;" src="images/apply.jpg" width="64" onclick="'.$this->mateInstances[0].'.toAjaxTableEditor(\'submit_proposal\',\''.$row['ann_ID'].'\');" />';
            }
        }
        else if ($user->isLoggedIn && $this->Editor->action == 'view_row')
        {
    ...
    public function changeBgColor($rowSets,$rowInfo,$rowNum)
    {
        ...
        $result = $this->Editor->doQuery("SELECT * from ".$this->Editor->tableName." WHERE ann_ID = '".$rowInfo['ann_ID']."' AND ann_Date_Close >= NOW()");
    ...
    public function getUserIcons($info)
    {
        $iconHtml = '';
        $numIcons = 0;
        $iconHtml .= '<li class="submit-proposal"><a href="javascript: void(0);" onclick="'.$this->mateInstances[0].'.toAjaxTableEditor(\'submit_proposal\',\''.$info['ann_ID'].'\');" title="Submit Proposal"></a></li>';
        $numIcons++; 
        return array('icon_html' => $iconHtml, 'num_icons' => $numIcons);
    }
    
    public function submitProposal($ann_ID)
    {
        $this->Editor->addJavascript('window.location.href = "Proposals.php?ann_ID='.$ann_ID.'"');
    }
}
$lte = new OpenAnnouncements();
?>
