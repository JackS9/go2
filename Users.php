...
class Users extends Common
{
    ...
    protected function initiateEditor()
    {
        ...
        $tableColumns['sec_source_ID'] = array(
            'display_text' => 'User Name', 
            'join' => array('table' => 'tbl_MARS_People', 'column' => 'people_ID',
                'display_mask' => "concat(people_user.people_LastName,', ',people_user.people_FirstName)",
                'alias' => 'people_user',
                'type' => 'left'
            ),
            'perms' => 'EVTAXQSFHO',
        ...
        if($user->isLoggedIn && (!isset($_GET['read-only']) || $user->isAdmin || $user->isManager))
        {
            ...
            $tableColumns['people_Email'] = array(
                'display_text' => 'E-mail',
                'display_mask' => 'people.people_Email',
                'perms' => 'EVAXQSFHO',
                'input_info' => 'size=30',
                'req' => 'true'
            );
            $tableColumns['people_OfficePhone'] = array(
                ...
                'display_mask' => 'people.people_OfficePhone',
                'perms' => 'EVAXQSFHO'
            );
        ...
        $this->Editor->setConfig('orderByColumn','sec_ID');
        ...
        $this->Editor->setConfig('customJoin',"LEFT JOIN tbl_MARS_People AS people ON tbl_MARS_Sec.sec_source_ID = people.people_ID LEFT JOIN tbl_MARS_Inst AS inst ON people.people_Dept1 = inst.inst_ID LEFT JOIN tbl_MARS_Bio AS bio ON people.people_ID = bio.people_ID LEFT JOIN tbl_MARS_Demographic_Data AS demog ON people.people_ID = demog.demog_source_ID");
        ...
        if($user->isLoggedIn)
        {
            ...
            {
                $this->Editor->setConfig('sqlFilters',"sec_ID = '".(isset($_GET['Sec_ID']) ? $_GET['Sec_ID'] : NULL)."'");
            }
            if(!isset($_GET['asAdmin']) && !isset($_GET['asManager']))
            {
                $this->Editor->setConfig('sqlFilters',"sec_ID = '".$user->id."'");
            }
        }
    }
    ...
    public function displayHtml()
    {
        ...
        $defaultSessionData['orderByColumn'] = 'sec_ID';
