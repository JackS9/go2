...
var mate = (function(instanceName) {
	...
	return {
		...
		gatherInputInfo: function(mateRowNum) {
			...
			var info = {'has_file_upload': false};
			var self = this;
			...
			$('#' + formId + ' :input').each(function(i,inputElem) {
				...
				if(inputId.length > 0 && $(inputElem).prop('disabled') == false) {
					if($(inputElem).attr('type') == 'file') {
						info[inputId] = $(inputElem).val();
						if($(inputElem).val().length > 0 || self.mateSubmitEmptyUpload) {
		...
		updateRow: function(id) {
			...
			if(info['has_file_upload']) {
				var data = {'info': info, 'action': 'update_row'};
				$('<input/>', {
					id: 'json',
					name:'json',
					type: 'hidden',
					value: $.toJSON(data),
				}).appendTo('#' + instanceName + 'add_edit_form');
		...
		insertRow: function() {
			...
			var info = this.gatherInputInfo();
			if(info['has_file_upload']) {
				var data = {'info': info, 'action': 'insert_row'};
				$('<input/>', {
					id: 'json',
					name:'json',
					type: 'hidden',
			...
			if($('#' + instanceName+'header_row') != null) {
				...
				$(headerCells).each( function (index,td) {
					...
					if($(td).attr('filterCol')) {
						...
						var filterType = $(td).attr('filterType') ? $(td).attr('filterType') : '';
						inputWidth = cellWidth * .9;
						inputWidth = Math.round(inputWidth);
						if(filterType == 'menu') {
							html += '<select id="'+instanceName+'filter_'+filterCol+'" class="filterInput" filterCol="'+filterCol+'" type="text" style="width: '+inputWidth+'px;" value="'+filterStr+'" onChange="' + instanceName + '.handleFilterSearch()">';
							html += '<option></option>';
							html += '</select>';
						} else {
		...
		editInPlace: function(id,mateRowNum) {
			var info = {id: id, mateRowNum: mateRowNum};
			this.toAjaxTableEditor('edit_in_place',info);
		},
		
		cancelEditInPlaceOld: function(id,mateRowNum) {
			var info = {id: id, mateRowNum: mateRowNum, newRow: false};
			this.toAjaxTableEditor('display_row_in_place',info);
		},
		

		updateRowsInPlace: function(id,mateRowNum) {
			var uipRows = this.getUipRows(id,mateRowNum);
			var ajaxRows = [];
			var formRow = [];
			var id;
			var inputInfo;
			var counter = 0;
			for(var mateRowNum in uipRows) {
				id = uipRows[mateRowNum];
				this.moveFormInputs(mateRowNum);
				inputInfo = this.gatherInputInfo(mateRowNum);
				formRow[0] = {};
				formRow[0]['inputInfo'] = inputInfo;
				formRow[0]['old_primary_key_value'] = id;
				formRow[0]['mateRowNum'] = mateRowNum;
				if(inputInfo['has_file_upload']) {
					var data = {'info': {'rows': formRow}, 'action': 'update_rows_in_place'};
					$('<input/>', {
						id: 'json',
						name:'json',
						type: 'hidden',
						value: $.toJSON(data),
					}).appendTo('#' + instanceName + 'add_edit_form' + mateRowNum);
					this.submitFileUploadForm(mateRowNum);
				} else {
					ajaxRows[counter] = {};
					ajaxRows[counter]['inputInfo'] = inputInfo;
					ajaxRows[counter]['old_primary_key_value'] = id;
					ajaxRows[counter]['mateRowNum'] = mateRowNum;
					counter++;
				}
			}
			var info = {'rows': ajaxRows};
			if(ajaxRows.length > 0) {
				this.toAjaxTableEditor('update_rows_in_place',info);
			}
		},
		
		getUipRows: function(id,mateRowNum) {
			var uipRows = {};
			if(id == null) {
				$('#'+instanceName+'table_form tr.editInPlaceRow').each(function(index,row) {
						uipRows[$(row).attr('row_num')] = $(row).attr('row_id');
				});
			} else {
				uipRows[mateRowNum] = id;
			}
			return uipRows;
		},
		
		cancelRowsInPlace: function(id,mateRowNum) {
			var uipRows = this.getUipRows(id,mateRowNum);
			var numEditInPlaceRows = $('#'+instanceName+'table_form tr.editInPlaceRow').length;
			var info = {'remove_update_btns': false, 'uipRows': uipRows};
			if(Object.size(uipRows) == numEditInPlaceRows) {
				info['remove_update_btns'] = true;
			}
			this.toAjaxTableEditor('cancel_rows_in_place',info);
		},
		
		updateInPlaceBtns: function(addPermission,editPermission,multiplePermission,iconPermission,persistentAddForm) {
			$('#'+instanceName+'updateInPlaceLayer div.addInPlaceBtnsDiv').css('display','none');
			$('#'+instanceName+'updateInPlaceLayer div.editInPlaceBtnsDiv').css('display','none');
			if(editPermission && multiplePermission && $('#'+instanceName+'table_form tr.editInPlaceRow').length > 0) {
				$('#'+instanceName+'updateInPlaceLayer div.editInPlaceBtnsDiv').css('display','block');
			}
			if((addPermission && !persistentAddForm && $('#' + instanceName+'add_in_place_row').length > 0 && !iconPermission) || (addPermission && persistentAddForm && multiplePermission && !iconPermission)) {
				$('#'+instanceName+'updateInPlaceLayer div.addInPlaceBtnsDiv').css('display','block');
			}
		},
		
		editRowsInPlace: function() {
			var selectedRows = this.getSelectedRows();
			var numRows = Object.size(selectedRows);
			if(numRows == 0) {
				this.showTooltipMsg(mateSelectRow);
			} else {
				this.toAjaxTableEditor('edit_rows_in_place',selectedRows);
			}
		},
		
		cancelAddInPlace: function() {
			$('#' + instanceName+'add_in_place_row').remove();
			$('#'+instanceName+'updateInPlaceLayer div.addInPlaceBtnsDiv').css('display','none');
		},
		
		insertAddInPlaceForm: function(html,filterPermission,filterPosition) {
			if($('#' + instanceName+'add_in_place_row').length == 0) {
				if(filterPermission && filterPosition == 'bottom' && $('#' + instanceName+'filter_row').length > 0) {
					$('#' + instanceName+'filter_row').before(html);
				} else {
		...
		insertInPlace: function() {
			this.moveFormInputs();
			var info = this.gatherInputInfo();
			info['last_row_num'] = this.getLastRowNum();
			
			if(info['has_file_upload']) {
				var data = {'info': info, 'action': 'insert_in_place'};
				$('<input/>', {
					id: 'json',
					name:'json',
					type: 'hidden',
				...
				this.toAjaxTableEditor('insert_in_place',info);
			}
		},
		
		submitFileUploadForm: function(mateRowNum) {
			mateRowNum = mateRowNum == null ? '' : mateRowNum;
			//$('#' + instanceName+'add_edit_form'+mateRowNum).submit();
			document.forms[instanceName+'add_edit_form'+mateRowNum].submit();
			this.showTooltipMsg(mateUploading,'msg');
		},
		
		confirmDeleteUpload: function(uploadName,id,col,inputId) {
			if(confirm(mateDeleteUploadMsg.replace(/#upload_name#/,uploadName))) {
				var info = {};
				info.uploadName = uploadName;
				info.id = id;
				info.col = col;
				info.inputId = inputId;
				this.toAjaxTableEditor('delete_upload',info);
			}
		},
